...
<script>
  ...
  export default {
     ...
     */
    components: {
      JobApplicationHeader: () => import('../job-application-header/JobApplicationHeader'),
      BidirectionalStepper: () => import('../bidirectional-stepper/BidirectionalStepper'),
      JobApplicationFooter: () => import('../job-application-footer/JobApplicationFooter'),
    },
    ...
    computed: {
      ...
      setStoreInLocalStorage() {
        ...
        if (this.attemptedLocalStorageLoad) {
          localStorage.trJobApplication = JSON.stringify(this.$store.state.jobApplicationFormData);
    ...
    methods: {
      ...
      getStoreFromDatabase() {
        ...
        JobApplicationsService
            .get(this.$route.query.id, jobAppSecurityToken)
            .then((result) => {
              if (typeof result.data === 'undefined' || typeof result.data.JSON === 'undefined') {
                this.$swal(
                  'Error',
      ...
      getStoreFromLocalStorage() {
        try {
          ...
          if (typeof localStorage.trJobApplicationTimestamp !== 'undefined') {
            ...
            const expirationTimestamp = parseInt(localStorage.trJobApplicationTimestamp, 10);
            ...
            if (currentTimestamp > (expirationTimestamp + 259200)) {
              ...
              localStorage.trJobApplicationTimestamp = Math.floor(Date.now() / 1000);
            }
          } else {
            // set expiration timestamp if it doesn't exist
            localStorage.trJobApplicationTimestamp = Math.floor(Date.now() / 1000);
          ...
          if (typeof localStorage.trJobApplication !== 'undefined') {
            const jobApplicationFormData = JSON.parse(localStorage.trJobApplication);
